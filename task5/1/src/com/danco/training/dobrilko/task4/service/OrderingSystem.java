...
public class OrderingSystem {
		...
		for (Book orderBook : order.getBooks()) {
			...
			if ((orderBook.getAuthor()
				...
				&& (orderBook.getPrice() == storeBook
					.getPrice()) && (storeBook.isInStore())) {

			    store.deleteBook(storeBook);
			    order.setStatus(false);
			    order.setDate(Calendar.getInstance());
	...
	if (orderToExecute.equals(null)) {
	    throw new RuntimeException("Order with id "
		    + Integer.toString(orderId) + "doesn't exist.");
	}
    }
