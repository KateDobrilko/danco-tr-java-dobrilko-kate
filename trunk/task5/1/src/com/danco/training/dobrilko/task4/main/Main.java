...
import com.danco.training.dobrilko.task4.service.BookShop;
...
public class Main {
	...
	final String ORDERING_SYSTEM_PATH_WRITE = "src\\OrderingSystem.txt";
	final String REPLY_SYSTEM_PATH_WRITE = "src\\ReplySystem.txt";
	final String STORE_PATH_WRITE = "src\\Store.txt";
	Parser parser = new Parser();
	OrderingSystem os = parser.OSReadFromFile(ORDERING_SYSTEM_PATH_READ);
	...
	bs.getStore().getUnclaimedBooks().forEach((Book book)-> System.out.println(book.toString()));
	...
	Book newBook = new Book(replyBook.getName(), replyBook.getAuthor(),
		replyBook.getPrice(), replyBook.getId(), Calendar.getInstance()
			.get(Calendar.YEAR), Calendar.getInstance().get(
			Calendar.MONTH), Calendar.getInstance().get(
			Calendar.DATE), replyBook.getDateOfPublication().get(
			Calendar.YEAR), replyBook.getDateOfPublication().get(
			Calendar.MONTH), replyBook.getDateOfPublication().get(
			Calendar.DATE));
	...
	bs.getStore().deleteBook(bs.getStore().getBooks().get(0));
	System.out.println(bs.getStore().toString());
	
	System.out.println(bs.getReplySystem().toString());
	System.out.println("___Reply Addition:");
	Reply r = new Reply(new Book("Anna Karenina","Tolstoi",12.5,new GregorianCalendar(2001,1,3),545654));
	bs.getReplySystem().addReply(r);
	bs.getReplySystem().addReply(r);
	System.out.println(bs.getReplySystem().toString());
	System.out.println("___Order Addition:");
	ArrayList<Book> books = new ArrayList<Book>();
	books.add(new Book("Kolobok","People",55.1,new GregorianCalendar(2010,3,2),54654));
	...
	System.out.println(bs.getOrderingSystem().getSumOfExecutedOrders(2013,
		7, 2014, 5));
	System.out.println("___ Executed Orders:");
	bs.getOrderingSystem().getExecutedOrders(2013, 7, 2014, 5)
		.forEach((Order order) -> System.out.println(order.toString()));
	System.out.println("___ Executed Orders Number:");
	System.out.println(bs.getOrderingSystem().getNumberOfExecutedOrders(
		2013, 7, 2014, 5));
